    ...
    if os.path.exists(path):
        with open(path, encoding="UTF-8") as file:
            ...
            if data[1].endswith("K"):
                ...
                if is_audio:
                    cmd = f"yt-dlp --continue --embed-subs --no-warnings --hls-prefer-ffmpeg --prefer-ffmpeg --downloader aria2p -f {data[1]} -o {op_dir}/%(title)s.%(ext)s {yt_url}"
                else:
                    cmd = f"yt-dlp --continue --embed-subs --no-warnings --hls-prefer-ffmpeg --prefer-ffmpeg --downloader aria2p -f {data[1]}+bestaudio[ext=m4a]/best -o {op_dir}/%(title)s.%(ext)s {yt_url}"
...
async def handle_ytdl_playlist_down(e: MessageLike) -> None:
    ...
    if os.path.exists(path):
        ...
        if data[1].endswith("k"):
            ...
            if data[1] == "best":
                vidcmd = f"yt-dlp -i --continue --embed-subs --no-warnings --prefer-ffmpeg --downloader aria2p -f 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best' -o '{opdir}/%(playlist_index)s - %(title)s.%(ext)s' {url}"
            else:
                vidcmd = f"yt-dlp -i --continue --embed-subs --no-warnings --prefer-ffmpeg --downloader aria2p -f 'bestvideo[ext=mp4,height<={data[1]}]+bestaudio[ext=m4a]/best' -o '{opdir}/%(playlist_index)s - %(title)s.%(ext)s' {url}"
